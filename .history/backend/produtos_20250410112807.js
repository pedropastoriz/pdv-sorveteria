...
router.get('/produtos', (req, res) => {
    const filePath = path.join(__dirname, '../data/produtos.json'); // Ajuste para o local do JSON
    ...
    try {
        if (!fs.existsSync(filePath)) {
            ...
            return res.status(500).json({ erro: "Arquivo JSON não encontrado." });
        }

        const data = fs.readFileSync(filePath, 'utf8');
        console.log("Conteúdo bruto do arquivo JSON:", data);
